...
<div class="bg-blue-200 bg-cover bg-no-repeat bg-center" style="background-image: url({{
    ...
    <div class="w-5/6 min-h-[420px] mx-auto flex items-stretch py-10">
        <div class="flex-1 flex flex-col justify-end text-gray-200" style="text-shadow:2px 2px #000">
            <h1 class="text-5xl font-serif font-medium">{{ $datas['landing-title']?->value ?? 'Hi,' }}</h1>
            <p class="leading-8 text-xl font-sans">{{ $datas['landing-desc']?->value ?? 'Welcome to my website' }}</p>
        </div>
    </div>
</div>
<div class="w-full md:w-5/6 min-h-[600px] mx-auto py-10">
    ...
    <div class="flex items-start mt-10 gap-5 pt-5">
        <div class="flex-1">
        ...
        <div class="flex-1">
            ...
            <div class="grid grid-cols-3 gap-5 mb-5">
                ...
                <div class="col-span-2">
                    <a href="/articles/{{ $article->slug }}" class="text-xl text-gray-900 hover:text-blue-800 font-serif capitalize">{{ $article->title }}</a><br>
                    <span class="text-gray-600 text-xs"><i class="fa-regular fa-clock"></i> {{ $article->created_at->diffForHumans() }}</span>
                    <p class="mt-2 text-gray-800 line-clamp-5 text-sm">{{ $article->description }}</p>
                </div>
            </div>
            @endforeach
        </div>
    </div>
    <div class="flex items-start justify-between mt-10 gap-5">
        <div class="flex-1">
